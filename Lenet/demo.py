# -*- coding: utf-8 -*-
"""
Created on Thu Jun 28 21:43:26 2018
...
def train(model, train_set, test_set, save, n_epochs=100, valid_size=5000,
    ...
    # Optimizer
    optimizer = torch.optim.SGD(model_wrapper.parameters(), lr=lr, momentum=momentum,
                                nesterov=True, weight_decay=wd)
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[0.5 * n_epochs, 0.75 * n_epochs],
                                                     gamma=0.1)

    # Start log
    with open(os.path.join(save, 'Lenet_results.csv'), 'w') as f:   # w = write 
    ...
    for epoch in range(n_epochs):
        ...
        if valid_loader and valid_error < best_error:
            ...
            torch.save(model.state_dict(), os.path.join(save, 'Lenet_model.dat'))
        else:
            torch.save(model.state_dict(), os.path.join(save, 'Lenet_model.dat'))

        # Log results
        with open(os.path.join(save, 'Lenet_results.csv'), 'a') as f:
    ...
    model.load_state_dict(torch.load(os.path.join(save, 'Lenet_model.dat')))
    if torch.cuda.is_available() and torch.cuda.device_count() > 1:
...
def demo(data, save, valid_size=5000,n_epochs=50, batch_size=64, seed=None):
    """
